...
import {
  LogOut, Plus, Trash2, Save, Code2, Briefcase, Wrench,
  BarChart2, Lock, Eye, EyeOff,
  Layers, Pencil, X, Check, GripVertical, ArrowUp, ArrowDown
} from "lucide-react";
...
/* ─── Helpers ─────────────────────────────────────── */
const TABS = [
  { key: "portfolio", label: "Portfolio",  icon: Code2 },
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#050507] px-4">
        <div className="w-full max-w-sm">
          <div className="text-center mb-10">
            ...
            <p className="text-xs font-heading text-zinc-600 uppercase tracking-widest">Rafi ERP CMS</p>
          </div>
          ...
          <p className="text-center text-[10px] text-zinc-700 mt-6 font-heading">Default password: rafi2026</p>
        </div>
      </div>
    );
  }
      ...
      <header className="sticky top-0 z-30 flex items-center justify-between px-6 py-4 bg-[#050507]/95 backdrop-blur border-b border-zinc-900">
        <div className="flex items-center gap-3">
          <div className="w-7 h-7 rounded-lg bg-brand-purple/25 border border-brand-purple/40 flex items-center justify-center">
            <span className="text-brand-purple font-display font-black text-xs">R</span>
          </div>
          <span className="font-display font-black text-sm text-zinc-200 tracking-widest">ERP CMS</span>
        </div>
        <div className="flex items-center gap-3">
...
function PortfolioPanel({ data, setData }: { data: SiteData; setData: (d: SiteData) => void }) {
  ...
  function moveUp(index: number) {
    ...
    portfolio[index - 1] = temp;
    setData({ ...data, portfolio });
  }

  function moveDown(index: number) {
    if (index === data.portfolio.length - 1) return;
    const portfolio = [...data.portfolio];
    const temp = portfolio[index];
    portfolio[index] = portfolio[index + 1];
    portfolio[index + 1] = temp;
        ...
        {data.portfolio.map((p, idx) => (
          <div key={p.id} className="rounded-2xl bg-zinc-950/60 border border-zinc-800/60 overflow-hidden group">
            <div className="aspect-video relative">
              ...
              {/* Reordering Controls */}
              <div className="absolute top-2 right-2 flex gap-1 bg-black/75 p-1 rounded-md">
                <button
                  disabled={idx === 0}
                  onClick={() => moveUp(idx)}
                  className="p-1 text-zinc-400 hover:text-zinc-100 disabled:opacity-30 disabled:hover:text-zinc-400 cursor-pointer"
                  title="Move Up (Increase Priority)"
                >
                  <ArrowUp className="w-3.5 h-3.5" />
                </button>
                <button
                  disabled={idx === data.portfolio.length - 1}
